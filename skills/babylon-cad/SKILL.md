---
name: babylon-cad
description: "Babylon.js 8 3D engine development expertise with curated API patterns, code examples, and on-demand documentation access. Use when working with Babylon.js scenes, meshes, materials (PBR/Standard), cameras, lights, shadows, GUI (2D/3D), animations, thin instances, glTF/OBJ/STL loading, post-processing, procedural/code-built 3D models, parametric CAD, 3D-printing model generation, React + Babylon integration, tone mapping / color management (ACES, exposure, sRGB, 发黑, 过曝, washed out), screenshots / 截图 / 缩略图 thumbnail / offscreen rendering, z-fighting / depth precision / 深度闪烁, or any 3D rendering task. Covers: (1) Scene setup and engine initialization (WebGL/WebGPU), (2) Mesh creation, transforms, instancing, and merging, (3) PBR and Standard materials with textures, (4) Camera types (ArcRotate, Universal, Follow), (5) Lighting and shadows, (6) 2D/3D GUI with AdvancedDynamicTexture, (7) Animation system and animation groups, (8) Asset loading (glTF, OBJ, STL), (9) Performance optimization and profiling, (10) React integration: engine/scene lifecycle in components, dispose discipline, StrictMode double-mount, (11) Procedural modeling for CAD and 3D printing: building high-quality 3D models from primitives with animation-ready parent/pivot hierarchies, CSG2 booleans, lathes, extrudes, and custom VertexData, (12) Color management: tone mapping, exposure, texture color spaces, environment lighting symptoms."
---
...
- **[rendering-color-management.md](references/rendering-color-management.md)** - Tone mapping (ACES vs Standard), exposure/contrast, texture sRGB/linear color spaces, PBR + environmentTexture pairing, and a symptom decision tree for black / washed-out / overexposed renders. **Read this when a render's colors look wrong.**
- **[screenshot-capture.md](references/screenshot-capture.md)** - `CreateScreenshot` APIs, render-before-capture timing, readPixels Y-flip, transparent-background captures, and an offscreen RenderTargetTexture thumbnail pipeline. **Read this when capturing canvas images or building part thumbnails.**
- **[depth-precision.md](references/depth-precision.md)** - Camera near/far ratio discipline, unit scale vs. depth, `zOffset` for coplanar geometry, per-material `useLogarithmicDepth` costs, and WebGPU reverse depth. **Read this when surfaces flicker or z-fight.**
- **[rendering-troubleshooting.md](references/rendering-troubleshooting.md)** - Field-debugged failure patterns: back-face-only rendering after resize (RTT depth attachment loss), transparent contact-plane z-fight (depth pre-pass + polygonOffset), StrictMode double-mount vs. async texture upload ("no texture bound"), UMD decoders under native ESM. **Read this when a render bug survives the obvious fixes.**
- **[gui.md](references/gui.md)** - AdvancedDynamicTexture (2D GUI) with all common controls, containers, layout, events; plus 3D GUI (GUI3DManager, Button3D, HolographicButton) for XR / near interaction
