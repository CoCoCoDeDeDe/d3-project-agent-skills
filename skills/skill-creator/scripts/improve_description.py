...
by calling `kimi -p` as a subprocess (same pattern as run_eval.py — uses
the session's Kimi Code auth, no separate API key needed).
"""
...
def _call_kimi(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"kimi -p exited {result.returncode}\nstderr: {result.stderr}"
        )
...
def improve_description(
    ...
    """Call Kimi to improve the description based on eval results."""
    failed_triggers = [
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; `kimi -p` is one-shot, so we
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_kimi(shorten_prompt, model)
